...
class HrvMetricsRepositoryImpl @Inject constructor(
    ...
    override suspend fun getUnsyncedSessions(limit: Int): List<HrvMetrics> {
        return hrvMetricsDao.getUnsyncedSessions(limit).map { it.toDomain() }
    }

    override suspend fun getWaveformForSession(sessionId: String): List<RawSample> {
        return channelValuesDao.getWaveformForSession(sessionId).map { entity ->
            RawSample(timestamp = entity.timestamp, value = entity.value)
        }
    }

    override suspend fun markAsSynced(sessionIds: List<String>) {
        hrvMetricsDao.markAsSynced(sessionIds)
    }

    override suspend fun getUnsyncedCount(): Int {
        return hrvMetricsDao.getUnsyncedCount()
    }
