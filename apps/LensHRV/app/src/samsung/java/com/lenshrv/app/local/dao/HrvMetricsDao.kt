...
""")
    ...
    @Query("SELECT * FROM hrv_metrics WHERE isSynced = 0 LIMIT :limit")
    suspend fun getUnsyncedSessions(limit: Int): List<HrvMetricsEntity>

    @Query("SELECT COUNT(*) FROM hrv_metrics WHERE isSynced = 0")
    suspend fun getUnsyncedCount(): Int

    @Query("UPDATE hrv_metrics SET isSynced = 1 WHERE id IN (:sessionIds)")
